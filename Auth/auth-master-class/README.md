# FULL Authentication System in React Native with Clerk and Expo Router

<https://www.youtube.com/watch?v=vT03ndfr_-w&ab_channel=notJust%E2%80%A4dev>

<https://github.com/notJust-dev/ReactNativeAuthentication>

## Setup

```bash
npx create-expo-app@latest clerk-study --template

npm i @clerk/clerk-expo expo-secure-store
```

53:20: react-hook-form
01:16:10: zod
01:40:15: navigation
01:48:20: protected pages

02:01:30: Root Layout
  02:03:50: Auth Layout

02:08:00: create providers: for auth
